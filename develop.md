...
这个方案实现了纯事件驱动架构，完全移除了FastAPI中间层，既保留了Celery的企业级特性，又最大化了架构的简洁性和性能。**最重要的是采用了统一的单文件事件处理模式：无论是单SQL提交还是ZIP包提交，都被Java服务转换为相同格式的单文件处理事件，Worker只需处理一种事件类型，大大simplify了架构复杂度。同时通过文件路径传递避免了大SQL文件导致的消息过大问题**，是一个彻底解耦、架构统一且高效的改造方案。