...
class TestTasksAPI:
    def test_get_task_detail(self, client):
        """测试获取任务详情API"""
        # 这里需要先创建一个任务，然后查询
        # 由于需要完整的任务创建流程，这里只是测试接口结构
        response = client.get("/api/v1/tasks?task_id=test-task-id")
    ...
    def test_get_task_result(self, client):
        """测试获取任务结果API"""
        response = client.get("/api/v1/tasks/result?task_id=test-task-id")
    ...
    def test_get_task_lint_result_not_found(self, client):
        """测试获取不存在的任务Lint结果"""
        response = client.get("/api/v1/tasks/result/lint?task_id=non-existent-task-id")
    ...
    def test_get_task_lint_result_success(self, mock_get_lint_result, mock_get_task, client):
        ...
        response = client.get("/api/v1/tasks/result/lint?task_id=test-task-id")
