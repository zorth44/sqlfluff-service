...
REDIS_MISSING = """\
You need redis-py >= 4.1.0 with native cluster support in order to use \
the Redis cluster result store backend."""
...
class RedisClusterBackend(KeyValueStoreBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 尝试新格式和旧格式的配置键
        self.conn_params = (
            self.app.conf.get('redis_cluster_settings') or 
            self.app.conf.get('CELERY_REDIS_CLUSTER_SETTINGS') or
    ...
    def _new_chord_return(self, task, state, result, propagate=None,
        ...
        # Redis集群不支持管道，使用单独的命令
        client.rpush(jkey, self.encode([1, tid, state, result]))
        readycount = client.llen(jkey)
        totaldiff = client.get(tkey)
        client.expire(jkey, 86400)
        client.expire(tkey, 86400)
        ...
        try:
            ...
            if readycount == total:
                ...
                # Redis集群不支持管道，使用单独的命令
                resl = client.lrange(jkey, 0, total)
                client.delete(jkey)
                client.delete(tkey)
                try:
